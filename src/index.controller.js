					...
					(user) => {
						if (user) {
							...
							const mailData = {
								...
								subject: 'Verification from Zero Waste',
								text: message,
								html: '',
							};
							...
							console.log('null user object created', user);
							res.statusCode = 500;
							res.setHeader('Content-Type', 'application/json');
							res.json({
								status: 'User sign up failed',
								err: 'User sign up failed',
							});
						}
					},
					(err) => {
			...
			User.User.findOne({
				...
				.then((user) => {
					...
					if (user) {
						if (!user.isVerified) {
							var err = new Error(
								'Please verify your email from the link sent by Zero Waste',
							);
							err.status = 401;
							return next(err);
						}
						...
						if (user.password) {
							...
							var err = new Error(
								'You have registered with Google account. Please login with your Google account or signup with email/password',
							);
							err.status = 401;
							return next(err);
						}
