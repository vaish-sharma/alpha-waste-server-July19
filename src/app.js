...
var sanitiser = function (req, res, next) {
	console.log('=== SANITISER ===');
	req.body = _.mapValues(req.body, function (value) {
	...
	console.log('After sanitisation', req.body);
	next();
};
...
function auth(req, res, next) {
	...
	if (!(req.session.userId || req.session.agentId)) {
		console.log('no req.session.userId or req.session.authId');
