		...
		Agent.findOne({ email: req.body.email })
			.then(
				(user) => {
					if (user) {
						if (!user.isVerified) {
							var err = new Error('Please get your agent profile verified.');
							err.status = 401;
							return next(err);
						}
						//if password matches
						user
							...
							.then((user) => {
								...
								if (user === true) {
									...
									console.log('req.agentSession', req.session);
									console.log('req.agentSession.id', req.session.agentId);
									res.statusCode = 200;
									res.setHeader('Content-Type', 'application/json');
...
function logout(req, res, next) {
	if (req.session.agentId) {
		req.session.destroy(req.session._id);
