		...
		.then(
			(order) => {
				if (order) {
					...
					const oldOrderAmt = item.item_units * item.item_amt;
					...
					const newOrderAmt = req.body.item_units * req.body.item_amt;
					if (
						order.order_amt &&
						order.order_amt + (newOrderAmt - oldOrderAmt) >= 0
					) {
						order.order_amt =
							(order.order_amt ? order.order_amt : 0) +
							(newOrderAmt - oldOrderAmt);
						order.save().then(
							(order) => {
								res.statusCode = 200;
								res.setHeader('Content-Type', 'application/json');
						...
						res.setHeader('Content-Type', 'application/json');
						res.json({
							status: 'Total order amount is less than updated amount',
						});
					}
				} else {
					res.statusCode = 404;
			...
			(order) => {
				if (order) {
					if (order.items.id(req.params.itemId)) {
						...
						if (
							order.order_amt &&
							order.order_amt >= item.item_units * item.item_amt
						) {
							order.order_amt =
								(order.order_amt ? order.order_amt : 0) -
							...
							res.setHeader('Content-Type', 'application/json');
							res.json({
								status: 'Total order amount is less than amount of item',
							});
						}
					}
				} else {
		...
		.then(
			(order) => {
				if (order) {
					...
					order.order_amt =
						(order.order_amt ? order.order_amt : 0) +
						req.body.item_units * req.body.item_amt;
					order.save().then(
						(order) => {
							res.statusCode = 200;
							res.setHeader('Content-Type', 'application/json');
